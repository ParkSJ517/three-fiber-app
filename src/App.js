...
const InfoModal = ({ selectedCountry, onClose }) => {
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: `${countryInfo.name} Carbon Emissions`,
        data: countryInfo.emissions,
        fill: false,
  ...
  return (
    <div className="info-modal" 
      ...
      <div className="info-modal-content" style={{ zIndex: 1001 }}>
        <button className="close-button" onClick={onClose} style={{ float: 'right', fontSize: '16px' }}>X</button>
        <h2 style={{ fontSize: '18px', marginBottom: '10px' }}>{countryInfo.name}</h2>

        {/* 탄소 중립 정책들 표시 */}
        <h3 style={{ fontSize: '16px' }}>탄소 중립 정책:</h3>
        ...
        {/* 탄소 배출 그래프 표시 */}
        <h3 style={{ fontSize: '16px', marginTop: '10px' }}>Carbon Emissions Over Time:</h3>
